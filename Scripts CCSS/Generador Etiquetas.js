...
function mixColors(colorsArray){
  ...
  //ChosenJustOneIngredient
    ...
    console.log(`Just one ingredient, returning the selected color: "${colorsArray[0]}"`)
  ...
  //Chosen 2 or more ingredients
    ...
    console.log(`Sum: ${sum}`)
    //Norm
    let norm = Math.sqrt(Math.pow(sum[0],2) + Math.pow(sum[1],2) + Math.pow(sum[2],2))
...
function changeEtiquetaColors(){
  ...
  var choosenIngredients = choosenIngredients.filter(function(value, index, arr){ return value != "";})
